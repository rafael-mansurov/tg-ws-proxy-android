...
def _files_dir_candidates() -> list[Path]:
    """Каталоги, где main.py мог записать tgws_proxy_secret.hex (getFilesDir и app_storage_path)."""
    out: list[Path] = []
    seen: set[str] = set()
    ...
    try:
        ...
        # 1) Тот же путь, что у UI: PythonService уже выставлен до nativeStart (см. PythonService.run).
        PySvc = autoclass("org.kivy.android.PythonService")
        svc = PySvc.mService
        if svc is not None:
            add(Path(svc.getFilesDir().getAbsolutePath()))
        # 2) Activity filesDir — совпадает с main._secret_storage_path() при открытом UI.
        act = autoclass("org.kivy.android.PythonActivity").mActivity
        if act is not None:
            add(Path(act.getFilesDir().getAbsolutePath()))
    except Exception:
        pass
    # 3) p4a / Kivy helper (иногда другой корень, чем getFilesDir).
    try:
        from android.storage import app_storage_path

        add(Path(app_storage_path()))
    except Exception:
        pass
    return out
...
def _secret_from_app_file() -> Optional[str]:
    ...
    for base in _files_dir_candidates():
        try:
...
def _app_base_path() -> Optional[Path]:
    for p in _files_dir_candidates():
        try:
            if p.is_dir():
                return p
        except OSError:
...
def _run_proxy() -> None:
    ...
    if not secret:
        tried = ", ".join(str(p) for p in _files_dir_candidates()) or "(нет каталогов)"
        _svc_log(
            base,
            f"нет секрета — проверьте pythonServiceArgument и {SECRET_FILENAME}; каталоги: {tried}",
            level="ERR",
        )
        sys.exit(1)
